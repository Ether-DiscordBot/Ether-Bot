...
class MochiBot(commands.Bot):
    ...
    async def on_ready(self):
        print("Logged as {0.display_name}#{0.discriminator} !".format(self.user))
        ...
        # Connect to reddit with paw
        self.reddit_client = praw.Reddit(client_id=os.getenv('REDDIT_CLIENT_ID'),
                             client_secret=os.getenv('REDDIT_CLIENT_SECRET'), password=os.getenv('REDDIT_PASS'),
                             user_agent='Mochi DiscordBot', username=os.getenv('REDDIT_NAME'), check_for_async=False)
