...
class CardHandler:
    def __init__(self) -> None:
        ...
        self.exp_font = ImageFont.truetype(
            os.path.abspath("ether/src/fonts/whitneybold.otf"), 15
        )
        self.level_font = ImageFont.truetype(
            os.path.abspath("ether/src/fonts/whitneybold.otf"), 35
        )
        self.backgrounds = [Image.open("ether/src/backgrounds/0.png")]
        ...
        self.exp_xy1 = (
            ...
            self.img_size[1] - self.pseudo_top_padding - self.line_width / 2,
        )
    ...
    async def create_card(self, user, db_user):
        ...
        img.paste(
            pp, (self.pp_padding, self.pp_padding), pp_mask
        )  # Paste the circle mask on the profile picture
        ...
        draw.text(
            xy=(
                self.pseudo_left_padding
                + self.font.getsize(f"{user.name[:13]}")[0]
                + 5,
                self.pseudo_top_padding - self.font.size / 2,
            ),
        ...
        draw.text(
            xy=(
                self.level_padding + self.font.getsize("Level ")[0],
                self.blevel_y - self.level_font.size + 3,
            ),
