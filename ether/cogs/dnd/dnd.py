...
class DnD(commands.Cog, name="dnd"):
    ...
    async def infos(self, ctx: ApplicationCommand, _class: Option(str, name="class", description="Pick a class", choices=[
        ...
        if not r.ok:
            return await ctx.respond(embed=EtherEmbeds.error("Sorry, an error was occured!"))
        
        class_data = r.json()
        
        
        primary_abilities = [ab["ability_score"]["name"] for ab in class_data["multi_classing"]["prerequisites"]]
        embed = Embed(
        ...
        embed.add_field(name="📖 Proficiencies",
                        value=f"{', '.join(proficiencies)}",
                        inline=False)
        skills = [skill["name"][7:] for skill in class_data['proficiency_choices'][0]['from']]
        embed.add_field(name="💪 Skills",
                        value=f"Choose two from {', '.join(skills)}",
                        inline=False,)
        sub_classes = [sub["name"] for sub in class_data['subclasses']]
        embed.add_field(name="🗂️ Sub classes",
                        value=f"{', '.join(sub_classes)}")
        
        embed.set_footer(text="Powered by D&D", icon_url="https://img.icons8.com/color/452/dungeons-and-dragons.png")

        await ctx.respond(embed=embed)
    
    @_class.command(name="spells")
    async def spells(self, ctx:ApplicationCommand, _class: Option(str, name="class", description="Pick a class", choices=[
        OptionChoice("Barbarian", value="barbarian"),
        OptionChoice("Bard", value="bard"),
        OptionChoice("Clearic", value="cleric"),
        OptionChoice("Druid", value="druid"),
        ...
        spells_data = r.json()
        spells_list = [f"[{s['name']}](https://dndbeyond.com/spells/{s['index']})" for s in spells_data["results"]]
        
        embed = Embed(title=f"{_class.capitalize()}' spells", url=f"https://dndbeyond.com/spells/class/{_class}")
    ...
    async def spell(self, ctx: ApplicationCommand, spell: str):
        ...
        if r.status_code == 404:
            return await ctx.respond(embed=EtherEmbeds.error("Sorry, I don't found your spell!"))
        elif not r.ok:
            return await ctx.respond(embed=EtherEmbeds.error("Sorry, an error was occured!"))
        
        spell_data = r.json()
        
        embed = Embed(title=f"{spell_data['name']} spell", url=f"https://www.dndbeyond.com/spells/{spell_data['index']}")
        ...
        embed.add_field(name=f"🗂️ Sub classes", value=f"{', '.join(sub_classes)}", inline=False)
