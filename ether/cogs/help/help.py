...
class Help(discord.Cog):
    def __init__(self, client):
        ...
        self.cooldown = commands.CooldownMapping(type=commands.BucketType.user, original=commands.Cooldown(1, 5))
    ...
    async def help(self, ctx):
        ...
        for ext in set(self.client.cogs.values()):
            ...
            #     continue
            if ext.qualified_name in self.admin_cogs and not await ctx.author.guild_permissions.administrator:
                continue
        ...
        embed = Embed()
        embed.set_author(icon_url=self.client.user.avatar.url, name=f"{self.client.user.name} Help")
        embed.set_thumbnail(url=self.client.user.avatar.url)
        embed.add_field(name="Categories:", value="\n".join(desc_array) + "\n\u200b")
        
        if ctx.guild:
            embed.set_footer(text="You can also navigate through the dropdown below to view commands in each category.")
    ...
    def build_cog_response(self, cog):
        ...
        for i in range(0, len(commands), 25):
            embeds.append(Embed(title=f"{cog.help_icon} {cog.qualified_name} Commands", description="".join(commands[i:i+25])))

        group = pages.PageGroup(pages=embeds, label="Page Group Test", description="page group test", timeout=180.0, disable_on_timeout=True)
